...
MODEL_PATH = "/models/efficientnet-b0/efficientnetb0_fasterrcnn_final.pth"

MODELS = [
    ("EfficientNetB0_RCNN", "N/A", MODEL_PATH),
]
...
def main():
    ...
    # Calculate num_classes based on dataset (background + classes)
    num_classes = len(unique_classes) + 1

    for tag, _, ckpt in MODELS:
        ...
        try:
            ...
            # Clean THOP junk keys if present
            clean_state_dict = {
        ...
        # These now use the updated functions targeting model.backbone
        params_M = compute_params(model)
        macs_G = compute_macs(model)
        
        # FPS and mAP still need the full model
